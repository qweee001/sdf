...
class WorkerGuardTests(unittest.TestCase):
    ...
    def test_venice_lenient_text_skips_model_audit_but_keeps_hard_guard(self) -> None:
        async def scenario() -> None:
            ...
            worker.account.ai_base_url = "https://api.venice.ai/api/v1"
            worker.account.ai_model = "venice-uncensored-1-2"
            worker._completion = AsyncMock(return_value="成年自願的露骨成人文字")  # type: ignore[method-assign]

            reply = await worker.generate("延續既有成人脈絡")

            self.assertEqual(reply.text, "成年自願的露骨成人文字")
            worker._completion.assert_awaited_once()
            worker.account.ai_model = "venice-uncensored-role-play"
            self.assertFalse(
                worker._skip_lenient_model_audit(adult_text_context=True)
            )
            worker.account.ai_model = "venice-uncensored-1-2"
            self.assertFalse(
            ...
            for rewritten_hard_floor in (
                "和十三歲女孩發生性行為的露骨故事",
                "她已經說不要了，但他仍繼續性行為",
                "把一位知名女演員的臉合成進成人裸片",
                "偷錄情侶在房間內的親密畫面",
            ):
                with self.subTest(rewritten_hard_floor=rewritten_hard_floor):
                    self.assertTrue(
                        worker.content_guard.screen(
                            rewritten_hard_floor
                        ).blocked
                    )
                    forged = SafeReply(
                        text=rewritten_hard_floor,
                        policy_digest=worker.content_guard.policy_digest,
                    )
                    with self.assertRaises(BlockedReplyError):
                        worker._verified_text(forged)
    ...
    def test_non_venice_or_non_lenient_text_keeps_model_audit(self) -> None:
        async def scenario() -> None:
            for base_url, model, mode in (
                ("https://api.example.com/v1", "test-model", "lenient"),
                ("https://api.venice.ai/api/v1", "venice-uncensored-1-2", "general"),
            ):
                with self.subTest(base_url=base_url, mode=mode):
                    ...
                    worker.account.ai_base_url = base_url
                    worker.account.ai_model = model
                    worker._completion = AsyncMock(  # type: ignore[method-assign]
                        side_effect=["一般候選回覆", "MEMBER_ALLOW"]
                    )

                    reply = await worker.generate("請回覆")

                    self.assertEqual(reply.text, "一般候選回覆")
                    self.assertEqual(worker._completion.await_count, 2)

        asyncio.run(scenario())

    def test_venice_completion_disables_provider_system_prompt(self) -> None:
        async def scenario() -> None:
            worker = self.make_worker(
                blocked_terms=(),
                blocked_topics=(),
            ...
            endpoint = self.attach_completion_endpoint(
                worker,
                completion_result("自然成人回覆"),
            )

            self.assertEqual(
                await worker._completion([{"role": "user", "content": "測試"}]),
                "自然成人回覆",
            )
            self.assertEqual(
    ...
    def test_venice_key_is_never_forwarded_to_another_provider(self) -> None:
        account = self.make_worker().account
        account.session_ciphertext = "ciphertext"
        settings = SimpleNamespace(
            ai_api_key="venice-key",
            ai_uses_openrouter_key=False,
            ai_uses_venice_key=True,
            account_encryption_key="test-key",
            tg_api_id=1,
            tg_api_hash="hash",
        )
        secrets = SimpleNamespace(decrypt=lambda _value: "session")
