...
若要把資料庫內所有既有帳號一次改成 OpenRouter `x-ai/grok-4.20` 並把成人純文字
策略設為 `general`（一般），可在 Railway Variables 將
...
隨機設定不會修改群組權限、屏蔽詞、API Base URL、密鑰或媒體開關／配額；抽中的
成人純文字策略會同步產生相符性格，且固定要求成年、自願、尊重隱私及拒絕即停止。清除某
...
每個帳號都有可在控制台切換的四級成人純文字策略，穩定 API 值如下：

- `lenient`（寬鬆）：成年人、自願、純文字虛構且已有成人上下文時，可使用較直接
  詞彙與較高細節並自然延展；仍不會從普通話題突然升級。
- `general`（一般）：允許直接成人詞彙與中等細節，可依既有成人上下文延展一步。
- `restricted`（限制）：只允許輕度曖昧、暗示及非圖像化用詞，只被動短承接，
  不描述器官或性行為細節，也不主動升級。
- `strict`（嚴格）：不允許露骨成人詞彙或色情角色扮演，只可非露骨談交友、感情、
  界線與安全，不延展成人情境；新帳號預設使用此級。
...
販運、性暴力、騷擾或勒索、偷拍或未經同意私密內容、可識別真人色情或性深偽、
跟蹤開盒或個資及非法活動。任何模式都不把虛構文字互動視為現實接觸、指定行為或
私密素材分享的同意。

相容欄位 `adult_text_enabled` 仍保留為衍生值。只有舊欄位的輸入或舊資料會把
`adult_text_enabled=true` 映射為 `general`，把
`adult_text_enabled=false` 映射為 `strict`；新寫入會同步兩欄。SQLite schema version 9
新增 `adult_text_mode`，啟動遷移會冪等地正規化無效模式，並依模式修正舊 bool，
不清除帳號、Session、群組或記憶。
